...
def index(request):
    ...
    suggested_list = list(chain(*user_profile_list))

    return render(request, 'index.html', {'user_profile':user_profile, 'posts':feed_lists, 'suggestions_username_profile_list':suggested_list[:4]})
...
def search(request):
    ...
    if request.method == 'POST':
        username = request.POST['username']
        # __icontains = Searches DB for any user containing hte letters that were put into the search box in order
